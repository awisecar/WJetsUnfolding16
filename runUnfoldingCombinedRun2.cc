...
#include "UnfoldingCombinedRun2.h"
...
int main(int argc, char **argv){
    ...
    int year           = 9999;                  // Full Run 2 dataset
    ...
    std::cout << "\n >>>>> Executing UnfoldingCombinedRun2(\"" << lepSel << "\", \"" <<  year << "\", \"" <<  algo << "\", \"" << histoDir << "\", \"" << unfoldDir << "\", " << jetPtMin << ", " << jetEtaMax << ", " << variable << ", " << doNormalized << ", " << whichSyst << ", " << isClosureTest << ");" << std::endl;
    std::cout << " >>>>> Where UnfoldingCombinedRun2(lepSel, year, algo, histoDir, unfoldDir, jetPtMin, jetEtaMax, variable, doNormalized, whichSyst, isClosureTest)" << std::endl;

    UnfoldingCombinedRun2(lepSel, year, algo, histoDir, unfoldDir, jetPtMin, jetEtaMax, variable, doNormalized, whichSyst, isClosureTest);
